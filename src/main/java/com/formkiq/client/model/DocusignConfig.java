...
public class DocusignConfig {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("userId", "integrationKey", "rsaPrivateKey", "hmacSignature"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DocusignConfig.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in DocusignConfig is not found in the empty JSON string", DocusignConfig.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DocusignConfig.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `DocusignConfig` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("rsaPrivateKey") != null && !jsonObj.get("rsaPrivateKey").isJsonNull()) && !jsonObj.get("rsaPrivateKey").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `rsaPrivateKey` to be a primitive type in the JSON string but got `%s`", jsonObj.get("rsaPrivateKey").toString()));
      }
      if ((jsonObj.get("hmacSignature") != null && !jsonObj.get("hmacSignature").isJsonNull()) && !jsonObj.get("hmacSignature").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `hmacSignature` to be a primitive type in the JSON string but got `%s`", jsonObj.get("hmacSignature").toString()));
      }
  }
