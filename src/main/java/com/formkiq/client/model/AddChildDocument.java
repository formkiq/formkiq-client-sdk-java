...
public class AddChildDocument {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(path, width, height, deepLinkPath, contentType, checksumType, checksum, isBase64, content, tags, metadata);
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("path", "width", "height", "deepLinkPath", "contentType", "checksumType", "checksum", "isBase64", "content", "tags", "metadata"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AddChildDocument.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in AddChildDocument is not found in the empty JSON string", AddChildDocument.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AddChildDocument.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `AddChildDocument` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
