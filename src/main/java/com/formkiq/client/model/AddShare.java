...
public class AddShare {
  ...
  * @throws IOException if the JSON Object is invalid with respect to AddShare
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!AddShare.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AddShare is not found in the empty JSON string", AddShare.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AddShare.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AddShare` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
