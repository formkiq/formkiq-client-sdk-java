...
public class AttributeSchemaCompositeKey {
  ...
  public AttributeSchemaCompositeKey attributeKeys(@javax.annotation.Nullable List<String> attributeKeys) {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AttributeSchemaCompositeKey.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in AttributeSchemaCompositeKey is not found in the empty JSON string", AttributeSchemaCompositeKey.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AttributeSchemaCompositeKey.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `AttributeSchemaCompositeKey` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("attributeKeys") != null && !jsonObj.get("attributeKeys").isJsonNull() && !jsonObj.get("attributeKeys").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `attributeKeys` to be an array in the JSON string but got `%s`", jsonObj.get("attributeKeys").toString()));
      }
  }
