...
public class AddDocusignEnvelopesResponse {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("envelopeId"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AddDocusignEnvelopesResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in AddDocusignEnvelopesResponse is not found in the empty JSON string", AddDocusignEnvelopesResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AddDocusignEnvelopesResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `AddDocusignEnvelopesResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("envelopeId") != null && !jsonObj.get("envelopeId").isJsonNull()) && !jsonObj.get("envelopeId").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `envelopeId` to be a primitive type in the JSON string but got `%s`", jsonObj.get("envelopeId").toString()));
      }
  }
