...
public class GetDocumentSyncResponse {
  ...
  private List<GetDocumentSync> syncs;
  ...
  * @throws IOException if the JSON Object is invalid with respect to GetDocumentSyncResponse
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!GetDocumentSyncResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GetDocumentSyncResponse is not found in the empty JSON string", GetDocumentSyncResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!GetDocumentSyncResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetDocumentSyncResponse` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("syncs") != null && !jsonObj.get("syncs").isJsonNull()) {
        ...
        if (jsonArraysyncs != null) {
          ...
          if (!jsonObj.get("syncs").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `syncs` to be an array in the JSON string but got `%s`", jsonObj.get("syncs").toString()));
          }

          // validate the optional field `syncs` (array)
          for (int i = 0; i < jsonArraysyncs.size(); i++) {
            GetDocumentSync.validateJsonObject(jsonArraysyncs.get(i).getAsJsonObject());
          };
        }
      }
  }
