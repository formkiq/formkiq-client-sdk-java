...
public class TagSchemaCompositeKey {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TagSchemaCompositeKey
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!TagSchemaCompositeKey.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                    // element is null
        throw new IllegalArgumentException(String.format(
            "The required field(s) %s in TagSchemaCompositeKey is not found in the empty JSON string",
            TagSchemaCompositeKey.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!TagSchemaCompositeKey.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(
            "The field `%s` in the JSON string is not defined in the `TagSchemaCompositeKey` properties. JSON: %s",
    ...
    if (jsonObj.get("key") != null && !jsonObj.get("key").isJsonNull()
        && !jsonObj.get("key").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format("Expected the field `key` to be an array in the JSON string but got `%s`",
