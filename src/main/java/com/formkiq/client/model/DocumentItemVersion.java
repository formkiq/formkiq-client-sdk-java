...
public class DocumentItemVersion {
  ...
  public int hashCode() {
    return Objects.hash(path, insertedDate, lastModifiedDate, checksum, documentId, contentType,
        userId, contentLength, versionId, versionKey);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to DocumentItemVersion
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!DocumentItemVersion.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                  // element is null
        throw new IllegalArgumentException(String.format(
            "The required field(s) %s in DocumentItemVersion is not found in the empty JSON string",
            DocumentItemVersion.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!DocumentItemVersion.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(
            "The field `%s` in the JSON string is not defined in the `DocumentItemVersion` properties. JSON: %s",
