...
public class DocusignNotificationExpirations {
  ...
  public DocusignNotificationExpirations expireAfter(@javax.annotation.Nullable String expireAfter) {
  ...
  public DocusignNotificationExpirations expireEnabled(@javax.annotation.Nullable String expireEnabled) {
  ...
  public boolean equals(Object o) {
    ...
    DocusignNotificationExpirations docusignNotificationExpirations = (DocusignNotificationExpirations) o;
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("expireAfter", "expireEnabled", "expireWarn"));
   ...
   * @throws IOException if the JSON Element is invalid with respect to DocusignNotificationExpirations
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DocusignNotificationExpirations.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in DocusignNotificationExpirations is not found in the empty JSON string", DocusignNotificationExpirations.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DocusignNotificationExpirations.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `DocusignNotificationExpirations` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("expireAfter") != null && !jsonObj.get("expireAfter").isJsonNull()) && !jsonObj.get("expireAfter").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `expireAfter` to be a primitive type in the JSON string but got `%s`", jsonObj.get("expireAfter").toString()));
      }
      if ((jsonObj.get("expireEnabled") != null && !jsonObj.get("expireEnabled").isJsonNull()) && !jsonObj.get("expireEnabled").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `expireEnabled` to be a primitive type in the JSON string but got `%s`", jsonObj.get("expireEnabled").toString()));
      }
      if ((jsonObj.get("expireWarn") != null && !jsonObj.get("expireWarn").isJsonNull()) && !jsonObj.get("expireWarn").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `expireWarn` to be a primitive type in the JSON string but got `%s`", jsonObj.get("expireWarn").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DocusignNotificationExpirations.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DocusignNotificationExpirations' and its subtypes
       }
       ...
       return (TypeAdapter<T>) new TypeAdapter<DocusignNotificationExpirations>() {
           @Override
           public void write(JsonWriter out, DocusignNotificationExpirations value) throws IOException {
