...
public class GetWebhookTagsResponse {
  ...
  * @throws IOException if the JSON Object is invalid with respect to GetWebhookTagsResponse
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!GetWebhookTagsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GetWebhookTagsResponse is not found in the empty JSON string", GetWebhookTagsResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!GetWebhookTagsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetWebhookTagsResponse` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("tags") != null && !jsonObj.get("tags").isJsonNull()) {
        ...
        if (jsonArraytags != null) {
          ...
          for (int i = 0; i < jsonArraytags.size(); i++) {
            WebhookTag.validateJsonObject(jsonArraytags.get(i).getAsJsonObject());
          };
        }
      }
  }
