...
public class OpaPolicyItem {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("policy", "allRoles", "anyRoles", "excludedRoles", "attributes", "input"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OpaPolicyItem.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in OpaPolicyItem is not found in the empty JSON string", OpaPolicyItem.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OpaPolicyItem.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `OpaPolicyItem` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("allRoles") != null && !jsonObj.get("allRoles").isJsonNull() && !jsonObj.get("allRoles").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `allRoles` to be an array in the JSON string but got `%s`", jsonObj.get("allRoles").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("anyRoles") != null && !jsonObj.get("anyRoles").isJsonNull() && !jsonObj.get("anyRoles").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `anyRoles` to be an array in the JSON string but got `%s`", jsonObj.get("anyRoles").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("excludedRoles") != null && !jsonObj.get("excludedRoles").isJsonNull() && !jsonObj.get("excludedRoles").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `excludedRoles` to be an array in the JSON string but got `%s`", jsonObj.get("excludedRoles").toString()));
      }
