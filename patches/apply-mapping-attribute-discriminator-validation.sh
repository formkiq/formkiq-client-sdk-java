...
function print_replacement() {
  ...
  print "                return;"
  print "            default:"
  print "                throw new IOException(String.format(java.util.Locale.ROOT, \"Failed to lookup discriminator value `%s` for MappingAttribute. Possible values: CONTENT CONTENT_KEY_VALUE DATA_CLASSIFICATION MALWARE_SCAN MANUAL METADATA METADATA_EXTRACTION_RESULT\", discriminatorValue));"
  print "        }"
  print "    }"
}
